...
LABELS = {"nnunet_v1":"nnU-Net v1\n(3D CNN)","swinunetr":"SwinUNETR\n(3D Tx)",
          "segformer3d":"SegFormer3D\n(3D Tx)","mednext_s":"MedNeXt-S\n(3D CNN)",
...
bars = ax.bar(range(len(order)), means, yerr=stds, capsize=5,
              color=[COLORS[n] for n in order], edgecolor="black", linewidth=0.5)
ax.set_xticks(range(len(order)))
ax.set_xticklabels([LABELS[n] for n in order], fontsize=10)
ax.set_ylabel("Mean per-case Dice (3-seed avg ± std)")
ax.set_title("Overall segmentation quality — per-case mean Dice (39 test cases)")
ax.set_ylim(0.90, 0.98)
for bar, m in zip(bars, means):
    ax.text(bar.get_x()+bar.get_width()/2, m+0.001, f"{m:.4f}", ha="center", va="bottom", fontsize=9)
...
ax.set_yticks(range(len(NETS))); ax.set_yticklabels([LABELS[n].replace("\n"," ") for n in NETS])
...
for idx, (c, title) in enumerate([(4,"left_kidney"),(5,"right_kidney")]):
    ...
    bp = ax.boxplot(data, labels=[LABELS[n].replace("\n"," ") for n in NETS], showfliers=True,
                    patch_artist=True, widths=0.6)
    for patch, n in zip(bp["boxes"], NETS):
        patch.set_facecolor(COLORS[n]); patch.set_alpha(0.7)
    ax.set_title(f"{title} — Dice distribution (n=117)")
    ax.set_ylabel("Dice"); ax.set_ylim(0, 1.0)
    ax.axhline(y=np.median(data[1]), color="gray", linestyle="--", alpha=0.4)  # swinunetr median ref
    ax.grid(axis="y", alpha=0.3)
    # annotate P10
    for i, d in enumerate(data):
        ax.text(i+1, np.percentile(d,10)-0.04, f"P10={np.percentile(d,10):.2f}", ha="center", fontsize=7, color="red")
...
for idx, (cls, title) in enumerate([(9, "head FP on TB cases\n(head absent — should be 0)"),
    ...
    ax.set_xticks(x); ax.set_xticklabels([LABELS[n].replace("\n"," ") for n in nets_o], fontsize=9)
...
ax.set_xticks(range(n)); ax.set_xticklabels([LABELS[x].replace("\n"," ") for x in nets3d], rotation=30, ha="right")
ax.set_yticks(range(n)); ax.set_yticklabels([LABELS[x].replace("\n"," ") for x in nets3d])
for i in range(n):
    for j in range(n):
