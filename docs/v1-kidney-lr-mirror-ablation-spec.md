...
Status: **ERRATA FIXED — 全部 axis 已修正，新训练已提交 (2026-06-24)**
Date: 2026-06-23 (original) / 2026-06-24 (errata + fix, 二次修正)
...
## Errata (2026-06-24): Audit 在 NIfTI 数据上做，但 nnU-Net 用预处理后数据，轴序不同

### 核心错误

Stage 0 的 axis audit 在 **NIfTI 原始数据**上做，但 nnU-Net 实际使用的是**预处理后的数据**，两者的轴序不同。预处理过程中发生了轴重排（NIfTI axis 0 ↔ axis 2 互换），但 plans 里记录的 `transpose_forward=[0,1,2]`（identity）与实际不符。

```
NIfTI (512,512,251):          预处理后 (251,656,656):
  axis 0 = LR (512)             axis 0 = CC (251)  ← NIfTI axis 2
  axis 1 = AP (512)             axis 1 = AP (656)  ← NIfTI axis 1
  axis 2 = CC (251)             axis 2 = LR (656)  ← NIfTI axis 0
```

### 后果

- Audit 得到 `confirmed_lr_axis_3d=0`（NIfTI axis 0=LR），但预处理后 LR 是 axis 2
- 3D SwinUNETR condlr 禁用了 axis 0（CC方向），**应该禁 axis 2（LR方向）**
- 2D nnU-Net condlr 禁用了 axis 0（H/AP方向），**应该禁 axis 1（W/LR方向）**
- 第一轮全部 6 个训练作业 + 第二轮 3 个 SwinUNETR 作业 = **共 9 个作业作废**

### 正确的 axis（预处理后数据）

| 维度 | 正确 axis | 预处理后方向 | Handoff 验证 |
|------|:---:|------|------|
| 3D | **2** | W/LR | handoff Task520 confirmed_lr_axis=2，fold4 kidney 显著改善 |
| 2D | **1** | W/LR (slice 内) | 预处理数据 CoM 分析 581:0 全票 |

### 修正（2026-06-24 已完成）

- 3D: `confirmed_lr_axis_3d` = **2**（DSUB 565391-565393，已提交）
- 2D: `confirmed_lr_axis_2d` = **1**（DSUB 565388-565390，已提交）
- 审计脚本 `tools/audit_lr_axis.py` 已增加 `audit_2d_on_preprocessed()` 函数（直接在预处理 npz 上做 2D 审计）
- Manifest: `tools/audit_output_v4/lr_axis_audit_manifest.json` (v3.1)

### 经验教训

**永远在 nnU-Net 实际使用的预处理数据上验证 axis 映射**，不要假设 NIfTI 轴序 = 预处理后轴序。Handoff 的审计得到了正确的 axis 2，因为它是在预处理数据上做的。这次错误导致 ~90 GPU-hours 浪费。
...
- 网络 = nnU-Net 2D + SwinUNETR 两个(**⚠️ ERRATA 2026-06-24: 全部 axis 已修正,3D axis=2,2D axis=1,新作业 565388-565393 已提交**);
- 论文不 claim "架构无关/普适"；
