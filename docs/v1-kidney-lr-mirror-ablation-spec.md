...
Status: **ERRATA FIXED — 2D audit corrected, new training submitted (2026-06-24)**
Date: 2026-06-23 (original) / 2026-06-24 (errata + fix)
...
**修正（2026-06-24 已完成）**:
- `confirmed_lr_axis_2d` = **1**（W/LR 方向，预处理数据上 581:0 全票通过）
- 2D condlr 训练/预测/评估结果应**废弃**（用了错误 axis）
- ~~需要重新训练 2D condlr~~ → **已完成**，DSUB 565385-565387 (`conditional_mirror_axis=1`)
- 3D SwinUNETR 不受影响（`mirror_axes=(0,1,2)` 包含所有轴）
- 参见修正后的 audit: `tools/audit_output_v4/lr_axis_audit_manifest.json` (v3)
...
- 网络 = nnU-Net 2D + SwinUNETR 两个(**⚠️ ERRATA 2026-06-24: 2D audit 已修正,LR axis=1,新作业 565385-565387 已提交**);
- 论文不 claim "架构无关/普适"；
