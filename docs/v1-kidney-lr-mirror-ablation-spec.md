...
## Errata (2026-06-24): 2D nnU-Net arm is FUNDAMENTALLY INAPPLICABLE

**错误**: Stage 0 的 2D axis audit 测试了轴向切面(沿 NIfTI axis 2 = CC/depth)，但 nnU-Net 2D 实际切片方向由 spacing 决定——对本数据集，spacing=(5.0, 0.977, 0.977)mm，**切片轴 = axis 0 (LR)**。axis 0 被 Convert3DTo2DTransform 合并到 batch 维度，2D slice 内的空间维度是 axis 1 (AP) 和 axis 2 (CC)。

**后果**: `mirror_axes=(0,1)` 在 2D 中翻转的是 AP 和 CC，**LR 轴从未被 mirror**。我们设置 `conditional_mirror_axis=0` 实际禁用的是 AP mirror，非 LR mirror。2D condlr 臂无效。

**修正**:
- `confirmed_lr_axis_2d = N/A`（LR 是切片轴，不在 mirror set 中）
- 2D condlr 训练和评估结果应**废弃**
- 仅 **SwinUNETR（3D）** 臂提供有效证据
- 参见修正后的 audit: `tools/audit_output_v2/lr_axis_audit_manifest.json` (v2.1)

**对 scope 的影响**: 原定 2 网络的阵容变为 **1 网络（SwinUNETR only）**。论文不应 claim 2D nnU-Net 的结果。
...
- 网络 = nnU-Net 2D + SwinUNETR 两个(**⚠️ ERRATA 2026-06-24: 2D nnU-Net 臂已废弃，LR 轴是该架构的切片轴不在 mirror set 中**);
- 论文不 claim "架构无关/普适"；实际有效阵容降为 **SwinUNETR only**；
