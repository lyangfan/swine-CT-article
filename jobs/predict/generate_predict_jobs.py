...
CKPT={ARTICLE}/data/nnunetv1/v1_comparison_2d_root/seed{seed}/runs/nnunet_2d__seed{seed}/fold_0/model_final_checkpoint.model
INPUT={ARTICLE}/data/nnunetv1/nnUNet_raw_data/Task601_Article622_Carcass9Class/imagesTs
OUT={ARTICLE}/data/nnunetv1/v1_comparison_predictions/nnunet_2d__seed{seed}
mkdir -p "${{OUT}}"
cd "{ARTICLE}"
echo "=== predict nnunet_2d seed={seed} (framework.predict, 2D, TTA off, overlap 0.5) ==="
"${{NNUNETV1_PYTHON}}" -m framework.predict \\
    --network nnunet_v1 --network-dim 2d --seed {seed} \\
    --checkpoint "${{CKPT}}" \\
    --input-folder "${{INPUT}}" \\
    --output-folder "${{OUT}}" \\
    --step-size 0.5
